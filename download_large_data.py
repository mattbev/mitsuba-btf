...
def download_file(url, dst_dir):
    ...
    try:
        with urllib.request.urlopen(request) as web_file, open(
            dst_path, "wb"
        ) as local_file:
...
def main():
    ...
    if os.path.exists(file_path):
        ...
        download_file(
            "https://cg.cs.uni-bonn.de/btf/UBO2003/IMPALLA/ONEFILE/UBO_IMPALLA256.zip",
            ubo2003_dir,
        )
        time.sleep(1)
    ...
    if os.path.exists(file_path):
        ...
        print(f"  Download {file_path}...")
        download_file(
            "https://cg.cs.uni-bonn.de/btf/UBO2003/WOOL/ONEFILE/UBO_WOOL256.zip",
            ubo2003_dir,
        )
        time.sleep(1)
    ...
    if os.path.exists(file_path):
        ...
        print(f"  Download {file_path}...")
        download_file(
            "https://hdrihaven.com/files/hdris/skylit_garage_1k.hdr", cloth_dir
        )
        time.sleep(1)
